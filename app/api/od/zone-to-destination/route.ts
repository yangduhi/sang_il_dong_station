...
import { odZoneToDestinationResponseSchema } from "@/lib/schemas/responses";
...
export async function GET(request: NextRequest) {
  ...
  const response = await repository.getZoneToDestination(query);
  const parsed = odZoneToDestinationResponseSchema.parse(response);
